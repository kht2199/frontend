...
const CATEGORY_TYPES = ["Temperature", "Pressure", "Humidity", "Vibration", "Power"];
...
export function generateProperties(categoryId: number): PropertyRecord[] {
	return [
		...
		{ id: 4, property: "Check Interval (sec)", value: String(30 + categoryId * 2) },
		{ id: 5, property: "Retry Count", value: String(1 + (categoryId % 5)) },
		{ id: 6, property: "Enabled", value: categoryId % 3 === 0 ? "N" : "Y" },
		{ id: 7, property: "Priority", value: categoryId % 2 === 0 ? "High" : "Normal" },
	];
}
...
export function generateDetails(categoryId: number): DetailRecord[] {
	return [
		{ id: 1, item: "알림 메시지", value: `Alert for category ${categoryId}`, remark: "" },
		{ id: 2, item: "발송 간격", value: `${5 + categoryId}분`, remark: "최소 1분" },
		{ id: 3, item: "최대 발송 횟수", value: String(3 + (categoryId % 5)), remark: "0=무제한" },
		{ id: 4, item: "알림 등급", value: categoryId % 2 === 0 ? "긴급" : "일반", remark: "" },
		{ id: 5, item: "에스컬레이션", value: categoryId % 3 === 0 ? "사용" : "미사용", remark: "" },
	];
}
